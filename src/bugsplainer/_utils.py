...
def add_lang_by_task(target_str, task, sub_task):
    ...
    elif task in ['explain', 'pretrain', 'finetune']:
        target_str = target_str
    return target_str
...
def create_read_explain_examples(input_col: str):
    def read_explain_examples(filename, data_num):
        """Read examples from filename."""
        from typing import List
        from pandas.io.parsers.readers import read_csv

        if 'mcmd' in filename:
            raise Exception('Support for MCMD dataset is dropped')

        examples: List[Example] = []

        target_col_name = 'commit_message'
        df = read_csv(
            filename,
            usecols=[input_col, target_col_name],
            nrows=None if data_num == -1 else data_num,
            keep_default_na=False,
        )
        if input_col != 'patch':
            df[input_col] = df[input_col].str.strip()
        df[target_col_name] = df[target_col_name].str.replace(re.compile('\\s+'), ' ').str.strip()
        for i, row in df.iterrows():
            examples.append(Example(idx=i, source=row[input_col], target=row[target_col_name]))
        return examples

    return read_explain_examples
